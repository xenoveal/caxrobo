'''
To train the model & continuous learning.
One time process:
1. Get all time data (as far as possible).
2. Split to train & test.
3. Find the most optimized state based on backtest.
'''
...
logger = setup_logger('LearningController', 'logs/learning_controller.log')

class LearningController:
    def __init__(self, symbol: str = "BTC-USD", 
                 interval: str = "1h", days: int = 720):
        self.symbol = symbol
        self.interval = interval
        self.days = days

    def get_save_dataset(self, dataset_path: str) -> None:
        logger.info("Start function to get & save data..")
        from modules.yf import YahooFinance

        yf = YahooFinance()
        csv_data: pd.DataFrame = yf.fetch_hist_data(self.symbol, days=self.days, interval=self.interval)

        csv_data.to_csv(dataset_path)

    def analyze_states(self, data, states, model):
    ...
    def plot_results(self, data, states, model, portfolio_value):
    ...
    def bruteforce_backtest(self, to_predict, states, initial_balance, model) -> dict:
    ...
    def main(self):
        dataset_path = f"dataset/{self.symbol}-{self.days}-{self.interval}.csv"
        self.get_save_dataset(dataset_path)
        ...
        self.analyze_states(to_predict, states, hmm.model)
        ...
        self.plot_results(to_predict, states, hmm.model, best_backtest.portfolio_value)
